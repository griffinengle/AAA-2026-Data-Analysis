...
# Import data after exclusion from csv and Drop ID column to deidentify
# (Optional) Set criteria for age filter
df = pd.read_csv("/workspaces/codespaces-blank/data/research-data.csv")
df.drop("ID", axis=1, inplace=True)
df = df[df['Age'] < 6.0]

# Import full dataset from csv and Drop ID column to deidentify
dataset = pd.read_csv("/workspaces/codespaces-blank/data/aaa-dataset2.csv")
print(rp.summary_cont(dataset['WPS_Score'].groupby(dataset['gender'])))
...
def GeneratePlots(frame, dcol, icol):
    ...
    anova_model = ols(f"{dcol} ~ C({icol})", data=df).fit()
